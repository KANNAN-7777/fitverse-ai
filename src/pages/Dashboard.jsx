...
import { Flame, Zap, Award, Target, Play, ShieldCheck, Trophy } from 'lucide-react';
...
export default function Dashboard() {
  ...
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-8 pb-12">
      ...
      {/* Professional Profile & Hero Banner */}
      <div className="relative rounded-3xl overflow-hidden p-8 border border-white/10 shadow-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        ...
        <div className="relative z-10 flex items-center gap-5">
          <div className="relative">
            <img 
              src={`https://ui-avatars.com/api/?name=${user?.name || 'Tomer Kannan'}&background=ccff00&color=000&size=128&bold=true`} 
              alt="Profile" 
              className="w-20 h-20 rounded-2xl border-2 border-primary shadow-[0_0_25px_rgba(204,255,0,0.4)] object-cover"
            />
            <span className="absolute -bottom-2 -right-2 bg-primary text-black p-1 rounded-lg shadow-lg" title="Verified Elite Athlete">
              <ShieldCheck size={16} />
            </span>
          </div>
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="bg-primary/20 text-primary border border-primary/40 text-xs font-extrabold px-3 py-0.5 rounded-full uppercase tracking-wider flex items-center gap-1">
                <Trophy size={12}/> Elite Tier Athlete
              </span>
              <span className="bg-white/10 text-gray-300 text-xs font-semibold px-2.5 py-0.5 rounded-full border border-white/10">
                Verified Account
              </span>
            </div>
            <h1 className="text-3xl md:text-4xl font-black text-white">{user?.name || 'Tomer Kannan'}</h1>
            <p className="text-gray-300 text-xs md:text-sm mt-1 font-medium">Smart India Hackathon 2026 Lead Participant • Performance Index Active</p>
          </div>
        </div>
        ...
        <div className="relative z-10 bg-black/60 backdrop-blur-md p-5 rounded-2xl border border-white/10 w-full md:w-64 shadow-xl">
          ...
          <div className="text-[11px] text-gray-400 mt-2 flex justify-between font-medium">
            <span>Next Rank: Master</span>
            <span>{1000 - (xp % 1000)} XP to go</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { icon: <Flame className="text-orange-500" size={24} />, label: 'Current Streak', val: `${streak} Days` },
          { icon: <Zap className="text-yellow-400" size={24} />, label: 'Workouts Completed', val: '28 Sessions' },
          { icon: <Target className="text-red-500" size={24} />, label: 'Active Burn', val: '8,450 kcal' },
          { icon: <Award className="text-primary" size={24} />, label: 'Total XP Earned', val: xp }
        ].map((stat, i) => (
          <motion.div key={i} variants={item} whileHover={{ y: -5 }} className="glass p-5 rounded-2xl flex flex-col justify-between border border-white/10 shadow-xl bg-black/40 backdrop-blur-md">
            <div className="mb-4 bg-white/5 w-fit p-3 rounded-xl border border-white/5 shadow-inner">{stat.icon}</div>
            <div>
