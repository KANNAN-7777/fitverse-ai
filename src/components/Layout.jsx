...
export default function Layout() {
  ...
  return (
    <div className="flex h-screen bg-darker text-white overflow-hidden">
      ...
      {/* Main Content Area */}
      <main className="flex-1 overflow-y-auto bg-darker p-4 md:p-8 pb-24 md:pb-8">
      ...
      <nav className="md:hidden fixed bottom-0 w-full glass flex justify-around p-4 border-t border-gray-800 z-50">
        {navItems.map((item) => (
          <NavLink key={item.path} to={item.path} end={item.path === '/app'} className={({ isActive }) => `p-2 rounded-full transition-colors ${isActive ? 'text-primary' : 'text-gray-500 hover:text-gray-300'}`}>
            {item.icon}
          </NavLink>
        ))}
      </nav>
